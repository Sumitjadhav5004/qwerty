...
while(temp!=NULL)
{
cout<<"\n"<<temp->rollno<<" \t "<<temp->name<<" \t "<<temp->userType<<"
\t\t "<<temp->year;
temp=temp->next;
}
}
...
else if(head->next==NULL && head->userType=='P')
{
delete head;
head=NULL;
}
...
if(head==NULL)
cout<<"\nCan't Delete ! No members Available !";
else if(head->next==NULL && head->userType=='S')
{
delete head;
head=NULL;
}
...
break;
case 3:
cout<<"\nEnter Rollno after which u want
to insert: ";
cin>>rollno;
